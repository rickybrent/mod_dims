...
dims_config_set_client(cmd_parms *cmd, void *d, int argc, char *const argv[])
{
    ...
    if(argc >= 1) {
        ...
        client_config->watermark_path = NULL;
        ...
        switch(argc) {
            case 9:
                if(strcmp(argv[8], "-") != 0) {
                    client_config->no_image_url = argv[8];
                }
            case 8:
                if(strcmp(argv[7], "-") != 0) {
...
static const char *
dims_config_set_watermark_path(cmd_parms *cmd, void *dummy, const char *arg)
{
    dims_config_rec *config = (dims_config_rec *) ap_get_module_config(
            cmd->server->module_config, &dims_module);
    config->watermark_path = (char *) arg;
    return NULL;
}
...
dims_handle_request(dims_request_rec *d)
{
    ...
    if(d->client_config && d->client_config->watermark_path) {
        d->watermark_path = d->client_config->watermark_path;
    }
...
dims_handler(request_rec *r)
{
    ...
    d->watermark_path = d->config->watermark_path;
    d->use_no_image = 0;
    d->image_url = NULL;
    d->filename = NULL;
